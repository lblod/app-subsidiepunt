defmodule Dispatcher do
  ...
  #################################################################
  # Subsidie management (insight)
  #################################################################
  # Frontend

  get "/assets/*path",  %{ accept: %{ any: true }, reverse_host: ["insight" | _rest] }  do
    forward conn, path, "http://frontend-subsidie-management/assets/"
  end

  get "/@appuniversum/*path", %{ accept: %{ any: true }, reverse_host: ["insight" | _rest] } do
    forward conn, path, "http://frontend-subsidie-management/@appuniversum/"
  end

  match "/*_path", %{ accept: %{ html: true }, reverse_host: ["insight" | _rest] } do
