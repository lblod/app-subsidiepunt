...
module.exports = {
  ...
  execute: async (store, graphs, lib, source) => {
    ...
    const attachmentFinancialReport = new $rdf.NamedNode(URI_BASE + mu.uuid());
    ...
    store.add($rdf.sym(source.uri), SUBSIDIE('attachmentFinancialReport'), $rdf.sym(attachmentFinancialReport), graphs.additions);
    store.add($rdf.sym(attachmentFinancialReport), RDF_TYPE, ATTACHMENT('FormData'), graphs.additions);
